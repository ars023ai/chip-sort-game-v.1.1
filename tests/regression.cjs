...
(async()=>{
 ...
 await page.evaluate(()=>{const d=ChipStackDebug;d.game.board=Array.from({length:30},(_,i)=>[4,9,14,22,27,29].includes(i)?[]:[(i+1)%6,(i+1)%6,(i+2)%6,i%6,i%6,i%6]);d.game.tray=[[1,1,2,0,0],[0,0,3,2,2],[2,2,1,3,3]];});
 ...
 await m.evaluate(()=>{const d=ChipStackDebug;d.game.board=Array.from({length:30},(_,i)=>[4,9,14,22,27,29].includes(i)?[]:[(i+1)%6,(i+1)%6,(i+2)%6,i%6,i%6,i%6]);d.game.tray=[[1,1,2,0,0],[0,0,3,2,2],[2,2,1,3,3]];});await m.screenshot({path:'/tmp/caps-mobile.png'});assert.deepEqual(errors,[]);
